---
name: loop-library
description: "Discover, find, compare, audit, repair, adapt, design, run, debrief, save, and publish repeatable AI-agent loops with explicit triggers, actions, verification, stopping conditions, guardrails, and handoffs. Use when the user asks to analyze code or coding threads for recurring work, find a published loop, turn an outcome into a bounded copy-ready loop, review a loop for weak checks or unsafe authority, execute a loop with an evidence receipt, learn from completed runs, save or reuse a project loop, or validate and submit a loop to Loop Library."
---
...
design a new one through a focused interview, run it with evidence, learn from
...
it. For another vague request, begin with: "What would you like the agent to
get done?"
...
feedback-cycle rules below before recommending or designing it.
...
When the user asks Loop Library to run, execute, or try a loop, read
...
Before finding or designing a loop in a project that contains `LOOPS.md`, read
