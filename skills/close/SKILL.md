...
1. Run `~/.codex/bin/sprintflow-scope.mjs assert-normal --ticket <ID> --repo <path> --branch <name>`. Any nonzero result refuses TROUT/epayment or ambiguous scope.
...
3. Normalize the closure snapshot and evaluate it with `~/.codex/bin/sprintflow-lifecycle-policy.mjs close --snapshot <json>`; its ownership and no-review-reentry limits are binding.
