...
External-ticket intake only. Read `~/.codex/skills/references/sprintflow-core.md` and `next-skill-router.md`.
...
1. Normalize the ticket ID. From the current cwd, run `~/.codex/bin/sprintflow-scope.mjs assert-normal --repo <repository-path> --ticket <normalized-ticket-id> --branch <active-branch>`. On any nonzero result, stop and recommend `$epayment-start` only for confirmed epayment; otherwise report ambiguity.
