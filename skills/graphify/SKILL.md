...
Output directory: `graphify-out/` by default, overridable per-project with the
`GRAPHIFY_OUT` env var. Every literal `graphify-out` below means "the output
directory" — resolve it as `${GRAPHIFY_OUT:-graphify-out}` when the current
project sets the var. troute-fulfillment sets `GRAPHIFY_OUT=.graphify-out`.
